...
class HostReactor extends Base {
  constructor(options = {}) {
    ...
    assert(
      options.serverProxy,
      "[HostReactor] options.serverProxy is required"
    );
  ...
  async _init() {
    await Promise.all([this.serverProxy.ready(), this._pushReceiver.ready()]);
  }

  processServiceJSON(json) {
    const data = JSON.parse(json);
    const serviceInfo = new ServiceInfo(data);
    const serviceKey = serviceInfo.getKey();
    if (serviceKey == null) {
      return null;
    }
    ...
    if (oldService) {
      if (oldService.lastRefTime > serviceInfo.lastRefTime) {
        this.logger.warn(
          "[HostReactor] out of date data received, old-t: %s, new-t: ",
          oldService.lastRefTime,
          serviceInfo.lastRefTime
        );
      }
      ...
      if (newHosts.length) {
        changed = true;
        this.logger.info(
          "[HostReactor] new ips(%d) service: %s -> %j",
          newHosts.length,
          serviceInfo.name,
          newHosts
        );
      ...
      if (remvHosts.length) {
        changed = true;
        this.logger.info(
          "[HostReactor] removed ips(%d) service: %s -> %j",
          remvHosts.length,
          serviceInfo.name,
          remvHosts
        );
      ...
      if (modHosts.length) {
        changed = true;
        this.logger.info(
          "[HostReactor] modified ips(%d) service: %s -> %j",
          modHosts.length,
          serviceInfo.name,
          modHosts
        );
      ...
      changed = true;
      this.logger.info(
        "[HostReactor] init new ips(%d) service: %s -> %j",
        serviceInfo.ipCount,
        serviceInfo.getKey(),
        serviceInfo.hosts
      );
    ...
    if (changed) {
      this.logger.info(
        "[HostReactor] current ips:(%d) service: %s -> %j",
        serviceInfo.ipCount,
        serviceInfo.getKey(),
        serviceInfo.hosts
      );
  ...
  subscribe(param, listener) {
    ...
    if (serviceInfo) {
      setImmediate(() => {
        listener(serviceInfo.hosts);
      });
    } else {
      this.getServiceInfo(
        param.serviceName,
        param.clusters || Constants.NAMING_DEFAULT_CLUSTER_NAME
      );
    }
  ...
  async getServiceInfoDirectlyFromServer(
    serviceName,
    clusters = Constants.NAMING_DEFAULT_CLUSTER_NAME
  ) {
    const result = await this.serverProxy.queryList(
      serviceName,
      clusters,
      0,
      false
  ...
  async updateServiceNow(serviceName, clusters) {
    try {
      ...
      this.logger.debug(
        "[HostReactor] updateServiceNow() serviceName: %s, clusters: %s, result: %s",
        serviceName,
        clusters,
        result
      );
  ...
  async refreshOnly(serviceName, clusters) {
    try {
      await this.serverProxy.queryList(
        serviceName,
        clusters,
        this._pushReceiver.udpPort,
        false
  ...
  _scheduleUpdateIfAbsent(serviceName, clusters) {
    ...
    const timer = setTimeout(() => {
      this._doUpdate(serviceName, clusters).catch((err) => {
        this.emit("error", err);
      });
    }, 1000);
    const task = {
  ...
  async _doUpdate(serviceName, clusters) {
    ...
    if (!serviceInfo || serviceInfo.lastRefTime <= task.lastRefTime) {
      ...
      this.logger.debug(
        "[HostReactor] refreshOnly, serviceInfo.lastRefTime: %s, task.lastRefTime: %s, serviceName: %s, clusters: %s",
        serviceInfo.lastRefTime,
        task.lastRefTime,
        serviceName,
        clusters
